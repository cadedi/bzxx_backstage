    ...
    >
      <el-form ref="stdForm" :model="std" label-width="120px">
        <el-form-item label="标准名称">
          <el-input v-model="std.name" />
        </el-form-item>
        <el-form-item label="标准号">
          <el-input v-model="std.standardNo" />
        </el-form-item>
        <el-form-item label="行业领域">
          <el-input v-model="std.industryType" />
        </el-form-item>
        ...
        <el-form-item label="标准类型">
          <el-select placeholder="选择标准类型" v-model="std.standardType">
            <el-option label="产品标准" value="产品标准"></el-option>
            <el-option label="基础标准" value="基础标准"></el-option>
            <el-option label="管理标准" value="管理标准"></el-option>
            <el-option label="方法标准" value="方法标准"></el-option>
            <el-option label="卫生标准" value="卫生标准"></el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async getInfos(page = 1) {
      ...
      const data = {
        keyword: searchObj.standardName,
        targetPage: page,
      };
      const result = await this.$API.industryStd.reqGetIndustryStd(data);
    ...
    async removeInfo(id) {
      await this.$API.industryStd.reqRemoveIndustryStd({ targetId: id });
    ...
    addOrUpdate() {
      ...
      if (std.id) {
        // 修改
        this.$API.industryStd.reqUpdateIndustryStd(std).then((result) => {
          this.loading = false;
        ...
        // 新增
        this.$API.industryStd.reqAddIndustryStd(std).then((result) => {
          this.loading = false;
          this.$message.success("保存成功!");
          this.getInfos(std.id ? this.page : 1);
          this.std = {};
