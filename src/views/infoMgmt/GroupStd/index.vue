    ...
    >
      <el-form ref="stdForm" :model="std" label-width="120px">
        <el-form-item label="团体名称">
          <el-input v-model="std.groupName" />
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async getInfos(page = 1) {
      ...
      const result = await this.$API.groupStd.reqGetGroupStd(data);
    ...
    async removeInfo(id) {
      await this.$API.groupStd.reqRemoveGroupStd({ targetId: id });
    ...
    addOrUpdate() {
      ...
      if(std.id){
        // 修改
          this.$API.groupStd.reqUpdateGroupStd(std).then((result) => {
          this.loading = false;
        ...
        // 新增
        this.$API.groupStd.reqAddGroupStd(std).then((result) => {
          this.loading = false;
          this.$message.success("保存成功!");
          this.getInfos(std.id ? this.page : 1);
          this.std = {};
