<template>
  <div class="app-container">
    ...
    <el-table border stripe v-loading="listLoading" :data="stdInfos">
      <el-table-column label="标准名称" width="250" align="center">
...
export default {
  ...
  methods: {
    ...
    async getInfos(page = 1) {
      ...
      const result = await this.$API.nationalStd.reqGetNationalStd(data);
    ...
    async removeInfo(id) {
      await this.$API.nationalStd.reqRemoveNationalStd({ targetId: id });
    ...
    addOrUpdate() {
      ...
      if (std.id) {
        // 修改
        this.$API.nationalStd.reqUpdateNationalStd(std).then((result) => {
          this.loading = false;
        ...
        // 新增
        this.$API.nationalStd.reqAddNationalStd(std).then((result) => {
          this.loading = false;
          this.$message.success("保存成功!");
          this.getInfos(std.id ? this.page : 1);
          this.std = {};
